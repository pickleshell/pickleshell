...
function start() {
  ...
  const service = new TerminalService(cfg);
    ...
    socket.on('data', async (chunk) => {
      ...
      while (buffer.includes('\n')) {
        ...
        try {
          ...
          const result = req.op === 'terminal-spawn' ? service.spawn(req) : req.op === 'terminal-write' ? service.write(req) : req.op === 'terminal-output' ? await service.output(req) : req.op === 'terminal-resize' ? service.resize(req) : req.op === 'terminal-signal' ? service.signal(req) : req.op === 'terminal-close' ? await service.closeRequest(req) : (() => { throw error('invalid_request'); })();
          response = result.ok === undefined ? { ok: true, ...result } : result;
        } catch (e) { response = { ok: false, error: e.code || 'internal_error', details: ['unauthorized', 'invalid_request', 'invalid_working_directory', 'executable_not_allowed', 'environment_not_allowed', 'signal_not_allowed', 'terminal_not_found', 'idempotency_conflict', 'terminal_not_writable', 'terminal_closed', 'idempotency_unsupported', 'input_too_large', 'output_limit', 'terminal_limit', 'terminal_spawn_failed', 'terminal_unavailable'].includes(e.code) ? e.message : 'request failed' }; }
        socket.write(`${JSON.stringify(response)}\n`);
      }
    });
  });
  server.listen(socketPath, () => { try { fs.chmodSync(socketPath, 0o660); } catch (_) {} });
  const shutdown = async () => { server.close(); await service.stop(); try { fs.unlinkSync(socketPath); } catch (_) {} process.exit(0); };
