...
async function main() {
  ...
  const closed = await service.closeRequest({ owner_scope: 'scope', chat_id: 'chat-test', terminal_id: spawned.terminal_id, reason: 'e2e_complete' }); assert.equal(closed.state, 'closed'); assert.equal(closed.close_reason, 'e2e_complete'); await assert.rejects(service.closeRequest({ owner_scope: 'scope', chat_id: 'chat-test', terminal_id: spawned.terminal_id, reason: 'bad reason' }), /reason is invalid/); assert.equal((await service.closeRequest({ owner_scope: 'scope', chat_id: 'chat-test', terminal_id: spawned.terminal_id })).already_closed, true); await service.stop(); fs.rmSync(root, { recursive: true, force: true });
