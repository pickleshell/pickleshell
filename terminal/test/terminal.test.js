...
const { spawnRequest, decodeData } = require('../src/validation');
...
async function main() {
  ...
  assert.throws(() => decodeData('!'), /invalid/); assert.throws(() => spawnRequest({ chat_id: 'x', executable: '/bin/false' }, makePolicy()), (e) => e.code === 'executable_not_allowed');
  ...
  const service = new TerminalService({ roots: [root], executables: ['/bin/sh'], maxTerminals: 1, ringBytes: 128, ttlMs: 60000 });
  const spawned = service.spawn({ owner_scope: 'scope', chat_id: 'chat-test', executable: '/bin/sh', argv: ['-c', 'printf exact; exit 3'], cwd: 'sub', cols: 80, rows: 24 }); assert.match(spawned.terminal_id, /^term_/); assert.throws(() => service.spawn({ owner_scope: 'scope', chat_id: 'other', executable: '/bin/sh', argv: [], cwd: 'sub' }), (e) => e.code === 'terminal_limit');
  const output = await eventually(async () => { const result = await service.output({ owner_scope: 'scope', chat_id: 'chat-test', terminal_id: spawned.terminal_id, cursor: 0, max_bytes: 128 }); return result.state === 'exited' ? result : null; }); assert.equal(Buffer.from(output.data, 'base64').toString(), 'exact'); assert.equal(output.exit_code, 3); assert.equal((await service.output({ owner_scope: 'scope', chat_id: 'chat-test', terminal_id: spawned.terminal_id, cursor: 0 })).data, output.data);
